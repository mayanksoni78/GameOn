...
const Cloud = ({ width, height, startY, speed, opacity, scale }: any) => {
  ...
  return (
    ...
    >
      {/* Pixel-art style composite cloud */}
      <View style={{ width: width * 0.7, height: height * 0.8, backgroundColor: '#BCAEEB', borderRadius: height / 2, position: 'absolute', bottom: 0, left: 0 }} />
      <View style={{ width: width * 0.6, height, backgroundColor: '#BCAEEB', borderRadius: height / 2, position: 'absolute', bottom: 0, left: width * 0.2 }} />
      <View style={{ width: width * 0.5, height: height * 0.7, backgroundColor: '#BCAEEB', borderRadius: height / 2, position: 'absolute', bottom: 0, right: 0 }} />
    </Animated.View>
  );
};
...
const Star = ({ startX, startY, size, delayTime, duration }: any) => {
  ...
  useEffect(() => {
    opacity.value = withRepeat(
      withSequence(
        withDelay(delayTime, withTiming(0.9, { duration: duration / 2, easing: Easing.inOut(Easing.ease) })),
...
export const AnimatedBackground = () => {
  ...
  // Seeded pseudo-random generator for consistent static buildings
  ...
  // Generate buildings
  const buildingsBG = useMemo(() => {
      return Array.from({ length: 30 }).map((_, i) => ({
          height: `${seededRandom(i * 10) * 15 + 10}%`,
          width: `${seededRandom(i * 11) * 8 + 4}%`,
          left: `${i * 3.5}%`,
          color: seededRandom(i * 12) > 0.5 ? '#0A051C' : '#0F0829',
      }));
  }, []);

  const buildingsFG = useMemo(() => {
      return Array.from({ length: 25 }).map((_, i) => ({
          height: `${seededRandom(i * 20) * 10 + 5}%`,
          width: `${seededRandom(i * 21) * 10 + 5}%`,
          left: `${i * 4.5}%`,
          color: seededRandom(i * 22) > 0.5 ? '#150A33' : '#1A0E40',
          hasFlicker: seededRandom(i * 23) > 0.4,
          flickerColor: ['#00E676', '#FF4081', '#2979FF', '#FFEA00', '#00E5FF'][Math.floor(seededRandom(i * 24) * 5)]
      }));
  }, []);
  ...
  return (
    <View style={[StyleSheet.absoluteFillObject, styles.fixedContainer]}>
      ...
      {/* Animated Stars - Dense & Tiny for premium look */}
      ...
      {/* Ambient Animated Clouds (Parallax effect with varying speeds and scales) */}
